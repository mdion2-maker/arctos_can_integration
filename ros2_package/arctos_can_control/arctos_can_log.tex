...
Both joints so far share one relationship worth testing further: the encoder
counting \emph{down} corresponded to clockwise on each. If that holds on a third
joint it is a far more useful rule than a byte per joint, since the encoder sign
is readable over the bus. Two samples is not enough to rely on it.

\subsection{X's endstop: found, but on the wrong input}
...
\textbf{It triggered \texttt{IN\_2}, not \texttt{IN\_1}.} \texttt{GoHome} homes to
the \emph{left} limit on \texttt{IN\_1}, so as wired the driver's own homing will
never see this switch. Either it is X's home switch on the wrong input and the
signal wire should move to \texttt{IN\_1}, or it is genuinely the far limit and a
second sensor exists counter-clockwise. Unresolved.
...
\subsection{Y: both limits read triggered at rest --- unresolved}
Y reports \texttt{0x0C}: both \texttt{IN\_1} and \texttt{IN\_2} low. That cannot
be physically true of endstops at opposite ends of travel, and with
\texttt{EndLimit} enabled it fences Y in from both directions at once.

The useful comparison is X, the same board type, whose \emph{unwired} inputs read
\textbf{high} (\texttt{0x0F}). So Y's inputs reading low is not simply ``nothing
connected'' --- something is holding them down. Either the sensors idle low and
assert high, inverted from C's, in which case Y needs \texttt{Hm\_Trig = High};
or there is a wiring fault. A magnet pass separates the two.

Y's usable travel is narrow, about $\pm20$\textdegree{}, so its search bounds must
be set accordingly --- the 30\textdegree{} and 180\textdegree{} sweeps used on X
are not safe here.
