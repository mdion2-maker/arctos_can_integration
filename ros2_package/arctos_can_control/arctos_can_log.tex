...
\subsection{Y's real envelope: 158.24\textdegree{}, and how much the gears slip}
\label{sec:y-envelope}
A first measurement with both limits located by movement gave
\textbf{165.5\textdegree{}}. \textbf{That figure was itself inflated by gear
slip}, and so was a later one. Repeating the calibration at different traverse
speeds produced a monotonic result:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Traverse speed} & \textbf{Measured separation} & \textbf{vs creep} \\ \midrule
150            & 178.550\textdegree{} & $+20.3$\textdegree{} \\
$\sim$50       & 165.540\textdegree{} & $+7.3$\textdegree{} \\
25 (run 1)     & 158.228\textdegree{} & --- \\
25 (run 2)     & \textbf{158.258\textdegree{}} & $+0.030$\textdegree{} \\
\bottomrule
\end{tabular}
\end{center}

The slower the traverse, the smaller the measured envelope --- which is exactly
what slip predicts. Slip means the motor turns \emph{more} than the joint, so
covering a fixed physical distance between two sensors needs extra motor
rotation, and the encoder, sitting upstream of the reduction, reports a distance
larger than reality.

\begin{caution}{Two creep runs agreeing to 0.030\textdegree{} is what makes this conclusive}
Slip is stochastic; it would not reproduce to three hundredths of a degree across
two independent full traverses, some 300\textdegree{} of travel in total. That
level of agreement means the gears are transmitting essentially perfectly at
speed 25, so \textbf{158.24\textdegree{} is the true envelope} and the faster
figures are measurement artefacts rather than alternative estimates.
\end{caution}

So the drivetrain is not broken, it has an operating envelope: \textbf{the gears
hold at speed 25 and slip progressively above it}, losing roughly
7\textdegree{} over a traverse at speed 50 and 20\textdegree{} at 150. That is
the question this joint was being run to answer. Runs at 50 and 100 would bracket
the onset more precisely; the method is proven and each run takes about six
minutes.

The original $\pm20$\textdegree{} estimate came from the session where the gears
were slipping badly enough that the arm barely moved at all, which is why the
envelope looked so narrow.

\begin{caution}{Approach both limits at the same speed, or the number is meaningless}
Trigger position varies with approach speed, and the two effects compound in the
same direction: a faster approach records the clockwise limit further clockwise
\emph{and} the counter-clockwise limit further counter-clockwise, inflating the
separation from both ends. The 165.5\textdegree{} figure mixed a speed-50 approach
with a speed-25 one. Latency alone is small --- 0.046\textdegree{} per trigger ---
so it cannot explain a 7\textdegree{} discrepancy, but it is one more reason to
fix the approach speed and keep it fixed.
\end{caution}

Thermally this was uneventful: roughly 150 motor revolutions across the two
calibration runs, twelve minutes of continuous travel, and the motor was still
cool to the touch afterwards. A clean drivetrain at this speed, not a marginal
one.
...
\subsection{Next steps}
\begin{itemize}
    \item Y is characterised (158.24\textdegree{} envelope,
    Section~\ref{sec:y-envelope}) but not homed or zeroed. Decide where its home
    should sit relative to \texttt{IN\_1} --- clear of the detection zone, as X's
    now is --- and set it with \texttt{0x92}.
    \item Bracket the slip onset with calibration runs at speed 50 and 100. The
    gears hold at 25 and lose $\sim$7\textdegree{} per traverse at 50, so the
...
\section{2026-09-11 --- Joint Z joins the bus; X and Y homed}
\label{sec:session-2026-09-11}
All three joints answered CAN: X \texttt{0x01}, Y \texttt{0x02}, Z \texttt{0x03}.
Z is an \texttt{MKS\_42D} at 150:1 --- the same board family as Joint C and the
same gear ratio as Y, which turned out to matter for both reasons.

\subsection{X and Y homed and zeroed}
X's endstop was found by supervised clockwise search
\textbf{290.1\textdegree{}} clockwise of the power-on position. It was
173.8\textdegree{} the previous session and 112\textdegree{} the one before ---
three sessions, three completely different offsets. The power-on reference is
arbitrary and the arm gets repositioned between sessions, so a stored trigger
position is only ever useful for choosing a search bound.

Y's homing procedure was defined for the first time, mirroring X's: approach
\texttt{IN\_1} clockwise at creep speed, back off 10\textdegree{}
counter-clockwise to clear its 2--4\textdegree{} detection zone, then
\texttt{SET\_AXIS\_ZERO}. The creep speed is part of the procedure, not a detail
--- a faster approach biases where the trigger is recorded and therefore where
home lands.

\begin{caution}{A homed zero does not survive a power cycle}
Later in the session the supply was cycled, and both X's and Y's
\texttt{raw = 0} references were gone --- the encoder counter resets to zero on
power-up regardless of what \texttt{0x92} was told earlier. Re-home after any
power interruption; a joint reading \texttt{0} is not necessarily at home.
\end{caution}

\subsection{Joint Z needed the limit port remap --- exactly like Joint C}
Z's sensor LED lit under a magnet while \texttt{READ\_IO} sat frozen at
\texttt{0x01} through several commanded moves. This is the \emph{second} time
this precise signature has appeared, and the cause was the same as on Joint C:
\textbf{on a 42D this build does not wire endstops to the \texttt{IN\_1}
terminal}. They go to \texttt{En} and \texttt{Dir} via limit port remap, which is
off by default, so bit 0 reports an empty terminal and idles high forever.

Sending \texttt{9E 01} changed the resting byte immediately, from \texttt{0x01}
to \texttt{0x02} --- bit 1 came alive because \texttt{Dir} is a real input under
remap. A magnet pass then showed bit 0 toggling \texttt{0x02}/\texttt{0x03} with
the magnet on and off, confirming the sensor sits on \texttt{En}, active low, so
the default \texttt{Hm\_Trig = Low} is correct.

\begin{caution}{\texttt{EndLimit} does not make an input visible}
It is natural to assume that enabling \texttt{EndLimit} is what makes an endstop
work, and that assumption cost time here. \texttt{EndLimit} governs only whether
the driver \emph{acts} on a limit --- stops the motor. Reading the pin through
\texttt{0x34} needs no \texttt{EndLimit} at all. If an input is not reporting,
\texttt{EndLimit} is not the missing piece; on a 42D, the remap almost certainly
is.
\end{caution}

The remap survived a power cycle, as it did on Joint C.

\subsection{Z has two endstops, and one of them was invisible}
Before the remap, sweeps counter-clockwise found nothing and Z looked like a
single-endstop joint in the way X genuinely is. That was an artefact:
\texttt{Dir} was not being read at all. With remap on, a counter-clockwise sweep
tripped \texttt{IN\_2} after 31.1\textdegree{}.

\begin{center}
\begin{tabular}{llll}
\toprule
\textbf{Input} & \textbf{Bit} & \textbf{Pin} & \textbf{Limit} \\ \midrule
\texttt{IN\_1} & 0 & \texttt{En}  & clockwise \\
\texttt{IN\_2} & 1 & \texttt{Dir} & counter-clockwise \\
\bottomrule
\end{tabular}
\end{center}

Both active low. Direction mapping confirmed by eye: \texttt{0x00} is clockwise
on Z, matching X and opposing Y and C --- still no cross-joint pattern.

\subsection{Z's envelope: 124.4\textdegree{}}
Two calibration runs, both approaches at speed 25:

\begin{center}
\begin{tabular}{lll}
\toprule
 & \textbf{Separation} & \\ \midrule
run 1 & 124.553\textdegree{} & \\
run 2 & 124.262\textdegree{} & difference \textbf{0.291\textdegree{}} \\
\bottomrule
\end{tabular}
\end{center}

That agreement rules out gross slip --- Y lost 7\textdegree{} per traverse at
speed 50, and this is 0.23\% over 124\textdegree{}. But it is ten times looser
than Y's creep-speed pair, which agreed to 0.030\textdegree{}. The individual
triggers each moved about a degree between runs (\texttt{IN\_1} by 1048 counts,
\texttt{IN\_2} by 939) and partly cancelled; both drifting the same rotational
way suggests the sensors are repeatable but the magnets arrive at slightly
different places, which is what a belt with some compliance would do. Record it
as \textbf{124.4\textdegree{} $\pm$ 0.3\textdegree{}}; a third run would show
whether the spread is scatter or one-directional drift.

\subsection{Belt fitted: a small, measurable cost}
The same 3\textdegree{} out-and-back before and after fitting Z's belt:

\begin{center}
\begin{tabular}{llll}
\toprule
 & \textbf{Out} & \textbf{Back} & \textbf{Net error} \\ \midrule
no belt   & $-2.903$\textdegree{} & $+2.901$\textdegree{} & 36 counts $=$ 0.005\textdegree{} \\
with belt & $-2.903$\textdegree{} & $+2.918$\textdegree{} & 162 counts $=$ 0.024\textdegree{} \\
\bottomrule
\end{tabular}
\end{center}

Both small. Note this is a motor-side measurement and cannot confirm the arm
moved.

\subsection{Two new scripts}
\begin{itemize}
    \item \texttt{joint\_move.py} --- \texttt{joint\_move.py z cw 15}. Three
    things to change: joint, direction, distance. Direction is given as a
    \emph{name} because the byte does not transfer between joints; the table
    holds each mapping confirmed by eye, and joints whose mapping has never been
    confirmed (A, B) are refused rather than guessed at.
    \item \texttt{joint\_calibrate.py} --- \texttt{joint\_calibrate.py z}. Sweeps
    to both limits at one speed and reports the separation. It imports the joint
    table from \texttt{joint\_move.py} so there is one table rather than two that
    drift apart.
\end{itemize}

\begin{caution}{The IO mask must follow the remap state, not the board type}
A bare 42D has only \texttt{IN\_1}, so bit 1 reads 0 permanently and must be
excluded from the abort logic or every 42D looks parked on an endstop. But a
\emph{remapped} 42D has two real inputs. \texttt{joint\_move.py} initially
hard-coded 42D as bit 0 only, and on Z --- remapped, with bit 0 already low
because a magnet was resting on the sensor --- it would have armed
\textbf{nothing} and moved the joint with no endstop protection at all, while
printing output that looked entirely normal. Fixed: the mask is now derived from
the remap flag.
\end{caution}

Also fixed: the cleanup path crashed with a \texttt{TypeError} when the driver
stopped answering mid-move, because it formatted a \texttt{None} status into the
final report. \texttt{0xF7} and the coil disable had already been sent, so the
joint was safe, but the exception skipped \texttt{bus.shutdown()}. A cleanup path
must never raise.

\subsection{Next steps}
\begin{itemize}
    \item Home and zero Z the way X and Y now are --- approach clockwise at creep
    speed, back off roughly 10\textdegree{} (its zone looks narrow, unlike X's
    10--17\textdegree{}), then \texttt{0x92}.
    \item Re-home X and Y: their zeros were lost to the power cycle.
    \item A third calibration run on Z if the $\pm0.3$\textdegree{} matters,
    to separate scatter from belt drift.
    \item Confirm A and B direction mappings before either can be driven ---
    \texttt{joint\_move.py} refuses them until then.
\end{itemize}

\section{2026-09-25 --- Joint A bare-motor bring-up, and a truncation bug in every move}
\label{sec:session-2026-09-25}
Joint A answered at \texttt{0x04}. It is an \texttt{MKS\_42D} at \textbf{48:1},
the coarsest ratio on the arm, and that coarseness is what exposed a bug that had
been present in every move this project has commanded.

\subsection{A fixed 0.129\textdegree{} was being cut off every move}
A's moves landed consistently short. The pattern is what identifies the cause:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Commanded} & \textbf{Delivered} & \textbf{Short by} \\ \midrule
4\textdegree{}   & 3.866\textdegree{}   & 0.134\textdegree{} \\
4\textdegree{}   & 3.875\textdegree{}   & 0.125\textdegree{} \\
180\textdegree{} & 179.871\textdegree{} & \textbf{0.129\textdegree{}} \\
\bottomrule
\end{tabular}
\end{center}

The \emph{same absolute} shortfall over a move forty-five times longer means a
fixed offset, not a proportional loss. Read as percentages it would have looked
like 97\% one time and 99.93\% the other, and neither reading suggests a bug ---
it took two very different distances to see it.

The cause was mine: \texttt{joint\_move.py} declared arrival once within 300
counts of target and exited, and the exit path sends \texttt{0xF7}, which
genuinely cancels the remainder. At A's 48:1 those 300 counts are
0.137\textdegree{}, matching the measurement almost exactly.

\begin{caution}{Why this hid on every other joint}
The same 300-count threshold is 0.026\textdegree{} on C at 67.82:1 and
0.011\textdegree{} on Y at 150:1 --- inside the noise. Only A's coarse 48:1 made
it large enough in joint terms to notice. A bug whose visibility scales with gear
ratio will always surface on the coarsest joint first, and will have been
silently present on the others the whole time.

It is harmless once and accumulates in a series: ten short steps on A would have
lost over a degree.
\end{caution}

Fixed by waiting for the encoder to stop moving before sending the stop, rather
than calling the move done early.

\subsection{Verification: a full turn each way}
Bare motor, nothing attached:

\begin{center}
\begin{tabular}{lll}
\toprule
 & \textbf{Delivered} & \texttt{raw} \\ \midrule
start                     & ---                     & 0 \\
360\textdegree{} clockwise & $+359.999$\textdegree{} & 786429 \\
360\textdegree{} counter-clockwise & $-360.006$\textdegree{} & $-18$ \\
\bottomrule
\end{tabular}
\end{center}

Round-trip error \textbf{18 counts, 0.0015\textdegree{}}, against the
0.129\textdegree{} the old code would have lost on each leg. Motion was smooth
and linear across 96 motor revolutions in both directions, no stalling and no
rough patches. This is a motor, driver and encoder result --- it says nothing
about a drivetrain, because there is none attached yet.

\subsection{Direction: another per-joint fact, not a derivable one}
Clockwise on A is \texttt{0x00}, confirmed by eye. Note that \texttt{0x00} counts
A's encoder \emph{up}, where the same byte counts \emph{down} on X and Z. So even
the relationship between encoder sign and direction byte is per joint. The table
in \texttt{Arctos\_CAN\_Interface} records what was observed; nothing about it is
derivable from another joint.

\texttt{joint\_move.py} gained \texttt{--assume-cw} for exactly this situation: it
moves a joint whose mapping has never been confirmed, states plainly that the
assumption is unverified, and says what to record afterwards. The refusal message
for an unmapped joint now names that path rather than simply declining. Only
Joint B is still unmapped.

\subsection{Endstop fitted, and not working: both poles lit}
A dual-output magnetic endstop was fitted to A. \textbf{Both the north and south
LEDs are lit at once, and neither changes when a magnet is brought near.}
\texttt{READ\_IO} is unchanged at \texttt{0x01}, exactly as before the sensor was
fitted, so the driver sees nothing either.

Both outputs asserted simultaneously is not a magnetic state --- a single die
cannot be presented with both poles at once --- so this is not detection, it is
both outputs stuck. Unresolved at the end of the session; see next steps.

\subsection{Two power losses, both mid-session}
Power dropped twice, each time shortly after an A move completed, with all four
drivers going silent and zero bus-errors. No motion was commanded either time:
the script's startup query got no reply and it exited before sending anything.
A was a bare motor drawing almost nothing, which makes a current-draw explanation
unlikely and points at the supply or a connector rather than the joint. Worth
watching whether it recurs.

The error handling introduced after the previous session earned its keep here:
the same situation crashed with a \texttt{TypeError} and leaked the CAN socket a
week earlier, and this time reported cleanly and refused to command a move to a
driver that was not listening.

\subsection{Next steps}
\begin{itemize}
    \item Diagnose A's endstop. Both LEDs lit with no response to a magnet points
    at supply rather than detection --- see the caution in
    \texttt{Arctos\_CAN\_Interface}'s endstop section.
    \item \textbf{A's remap is off} (\texttt{io = 0x01}). A dual-output sensor
    needs two inputs, and a bare 42D has one, so A will need \texttt{9E 01} and
    the \texttt{En}/\texttt{Dir} wiring with \texttt{Com} tied high --- the same
    arrangement as Z and C. Until then a second output has nowhere to go.
    \item A has never been coupled to its joint. Every result above is
    bare-motor.
    \item Joint B is the last unmapped direction.
\end{itemize}

\section{2026-09-25 (continued) --- Round-trip checks, and a stall detector that cried wolf}
\label{sec:session-2026-09-25b}

\subsection{A false-stall bug, and what gave it away}
A round-trip check across every joint reported three of four as stalling:
Joint A after 0.246\textdegree{}, Y after 21\textdegree{}, Z after 8.4\textdegree{}.
\textbf{The reading that exposed it was A's}, because A had completed a flawless
360\textdegree{} round trip minutes earlier. A joint does not develop a stall at
a quarter of a degree between one test and the next, so the detector was wrong,
not the arm.

The cause was reimplementation. \texttt{joint\_move.py} already had working stall
detection --- travel measured across a 1.5\,s window, and only after a 4\,s grace
period. Writing the round-trip script I wrote a cheaper one instead: consecutive
polls differing by under 5 counts, three times running, with no grace period at
all. At \texttt{ACCEL = 2} the acceleration ramp is slow enough that the first
few polls of every move look motionless, so it fired during the ramp.

\begin{caution}{Reuse the detector, do not rewrite it}
Both scripts now share \texttt{joint\_move.py}'s windowed logic. The bug was not
subtle physics --- it was writing a second version of something that already
worked, and the second version being worse. The same reasoning is why
\texttt{joint\_calibrate.py} imports the joint table rather than keeping a copy.
\end{caution}

The results from the faulty run are void. Only X's leg completed, and it is not
re-verified, so its $-0.47$\textdegree{} return error is recorded here as an open
question rather than a finding.

\subsection{Joint A: six round trips, bare motor}
\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Test} & \textbf{Return error} & \\ \midrule
360\textdegree{} each way, speed 50 & 0.0015\textdegree{} & \\
45\textdegree{}, speed 25 & 0.0206\textdegree{} & \\
45\textdegree{}, speed 25 & 0.0018\textdegree{} & \\
45\textdegree{}, speed 25 & 0.0027\textdegree{} & \\
45\textdegree{}, speed 25 & 0.0124\textdegree{} & \\
360\textdegree{} each way, speed 50 & \textbf{0.0005\textdegree{}} & one encoder count \\
\bottomrule
\end{tabular}
\end{center}

Every return inside 0.021\textdegree{}, most inside 0.003\textdegree{}, across
three power cycles and two speeds. On the best run the two legs matched each
other to 0.001\textdegree{} --- the joint travels the same distance in both
directions, which is the more telling property.

At speed 50 both legs overshot by about 0.07\textdegree{} where the speed-25 runs
landed within 0.02\textdegree{}: the driver carries further past target before
settling when it is moving faster. It is symmetric, so it cancels on a round
trip, which is why the return error is the figure to read rather than either leg.

\textbf{This remains a bare-motor result.} Nothing is attached to A. It proves
the motor, driver, encoder and command path, and says nothing about a
drivetrain --- which is precisely where Y looked perfect on the bus while its
gears lost 7\textdegree{} a traverse.

\subsection{Joint Z: the endstop did its job}
Z's 45\textdegree{} round trip stopped on \texttt{IN\_1} after
41.3\textdegree{} --- a genuine limit trigger, not a false stall --- then
returned the full 45.019\textdegree{}. The resulting $+3.745$\textdegree{}
displacement is arithmetic rather than a fault: $45.019 - 41.271 = 3.748$. The
summary flags a leg that stopped early for exactly this reason, so the number is
not mistaken for a measurement. Z's remap and both endstops survived the day's
power cycles.

\subsection{Per-joint speed limits are now enforced, not remembered}
Y's cycloidal gears slip above speed 25, and the encoder cannot see it happen.
That fact lived only in this log, where a caller had to remember it. It is now a
field in the \texttt{JOINTS} table and enforced by every consumer: asking for
speed 150 on Y clamps to 25 and says why. \texttt{None} in that field means
nobody has measured a limit --- it is not a default.

\subsection{Power dropped four times}
All four drivers went silent four times during the session, always with
\textbf{zero bus-errors} and the adapter still enumerated, which is the signature
of losing power rather than a bus fault. Every drop landed in the gap
\emph{between} runs, never during a move. Two of them followed A running as a
bare motor with nothing attached, drawing almost nothing.

That combination rules out current draw, the CAN bus and any single driver, and
points upstream at the supply or its connection --- something stable in steady
state that lets go shortly after a load event. Unresolved; it needs a meter and
eyes on the supply's own indicator at the moment it goes.

Worth recording that no completed run was corrupted: the drops only ever
prevented a run from starting, and A's accuracy either side of a power cycle is
indistinguishable.

\subsection{New script}
\texttt{joint\_roundtrip.py} --- \texttt{joint\_roundtrip.py 45}, or
\texttt{--joints x,z}. Moves each joint out and back and reports how far from its
start it finished. Joints that are not on the bus, or have no confirmed direction
mapping, are skipped rather than failed, so it works on a partly assembled arm.
A leg that stopped early on an endstop or a stall is flagged, because its return
error describes an interrupted move rather than a drivetrain.

\subsection{Next steps}
\begin{itemize}
    \item Re-verify X's $-0.47$\textdegree{} return error with the fixed
    detector. If it holds, X has roughly twenty times A's lost motion and that is
    worth understanding.
    \item A clean 30\textdegree{} round trip on Z, clear of its
    \texttt{IN\_1} limit.
    \item Chase the supply dropouts before they cost a long run.
    \item A's endstop is still dead from the reversed-supply damage; A also needs
    \texttt{9E 01} before a dual-output sensor has two inputs to report on.
\end{itemize}
