...
\textbf{The NEMA 17 figures do not transfer.} The NEMA 23 joints run a Servo57D,
where the NEMA 17 joints run a Servo42D --- different winding inductance and a
different torque-speed curve, so this motor's usable speed ceiling must be
measured, not assumed. The measured per-joint table in
\texttt{arctos\_motor\_guide.tex} is for Servo42D joints; do not read Joint B's
numbers across to a 57D.
