...
\subsection{Confirmed per-joint parameters}
\label{sec:joint-params}
Everything below was established on the machine, not read from a config file.
Blank means not yet confirmed --- treat it as unknown rather than assuming a
default.

\begin{center}
\begin{tabular}{lllllll}
\toprule
\textbf{J} & \textbf{Board} & \textbf{Gear} & \textbf{Remap} & \textbf{CW byte}
  & \textbf{Endstops} & \textbf{Envelope} \\ \midrule
X & 57D & 13.5   & no  & \texttt{0x00} & one, \texttt{IN\_2} (CW side) & --- \\
Y & 57D & 150.0  & no  & \texttt{0x80} & \texttt{IN\_1} CW, \texttt{IN\_2} CCW & 158.24\textdegree{} \\
Z & 42D & 150.0  & \textbf{yes} & \texttt{0x00} & \texttt{IN\_1}=\texttt{En} CW, \texttt{IN\_2}=\texttt{Dir} CCW & 124.4\textdegree{} $\pm$0.3 \\
A & 42D & 48.0   & no  & \texttt{0x00} &  & --- \\
B & 42D & 67.82  &     &               &  & --- \\
C & 42D & 67.82  & \textbf{yes} & \texttt{0x80} & on \texttt{En}/\texttt{Dir} & --- \\
\bottomrule
\end{tabular}
\end{center}

\begin{caution}{The clockwise byte does not transfer between joints}
\texttt{0x00} is clockwise on X and Z and counter-clockwise on Y and C. Four
joints, no pattern. An earlier draft proposed that ``encoder counting down means
clockwise'' held across joints; Y disproved it. Each mapping in the table was
established by watching the joint turn, and A and B are blank because nobody has
watched them yet. \texttt{joint\_move.py} refuses to move a joint whose mapping is
blank rather than guess, because a guess is a coin flip on direction.
\end{caution}

\begin{caution}{Per-joint quirks that have each cost a session}
\begin{itemize}
    \item \textbf{X has only one endstop}, on \texttt{IN\_2}. \texttt{GoHome}
    homes to \texttt{IN\_1}, so it would never terminate --- \textbf{never send
    \texttt{0x91} to X}.
    \item \textbf{Y's gears slip above speed 25}, losing $\sim$7\textdegree{}
    per traverse at 50 and $\sim$20\textdegree{} at 150. Calibrate it at 25.
    \item \textbf{Z and C are remapped}; their endstops are on \texttt{En} and
    \texttt{Dir}, not \texttt{IN\_1}, and \texttt{Com} must be tied high.
    \item \textbf{X's detection zone is 10--17\textdegree{} wide} but releases in
    0.062\textdegree{} going the other way, so home must sit clear of it. Y's and
    Z's are only a few degrees.
\end{itemize}
\end{caution}
...
\texttt{SET\_EN\_ACTIVE}    & \texttt{0x85} & \texttt{00} low, \texttt{01} high, \texttt{02} always on. \\
\texttt{SET\_AXIS\_ZERO}     & \texttt{0x92} & Zeroes the counter where it stands. No motion. \\
...
\subsection{Homing a joint in practice}
The procedure that has worked on X, Y and Z, and which does not depend on any
driver setting that cannot be read back:

\begin{enumerate}
    \item \textbf{Search} toward the chosen limit at a fixed slow speed, watching
    \texttt{0x34} directly and stopping on the bit yourself. Do not use
    \texttt{GoHome (0x91)}: its direction lives in \texttt{Hm\_Dir}, which cannot
    be queried, and the manual says the motor ``will keep running until it hits
    the limit switch'' --- with no switch on the watched input there is no
    terminating condition.
    \item \textbf{Record the trigger} position. That edge is the reference.
    \item \textbf{Step clear of the detection zone.} Parking inside it leaves the
    joint sitting on an asserted limit at its own home.
    \item \textbf{\texttt{SET\_AXIS\_ZERO (0x92)}} where it rests. Readings then
    become joint angles instead of offsets from an arbitrary power-on point.
\end{enumerate}

\begin{caution}{A homed zero does not survive a power cycle}
The encoder counter resets to zero at power-up regardless of what \texttt{0x92}
was told earlier, so a joint reading \texttt{0} is not necessarily at home ---
it may simply have just been powered on. Re-home after any power interruption.
This is the same reason a stored trigger position is only ever useful for
choosing a search bound: Joint X's endstop was 112\textdegree{} from the start
position one session, 173.8\textdegree{} the next and 290.1\textdegree{} the
one after.
\end{caution}

\subsection{Making a homing reference repeatable}
An endstop's trigger point is only a usable reference if it is approached the same
way every time. Two effects, both measured:

\textbf{Detection zones are asymmetric, and the sharp edge is directional.} A
magnet holds an input asserted over a band, not at a point. On Joint X that band
runs 10--17\textdegree{} wide, and backing out of it releases the input after only
\textbf{0.062\textdegree{}}. So the edge encountered on the way \emph{in} is
sharp and repeatable, while the far side is a plateau. Approaching from the other
direction trips somewhere in the band and gives a different answer each time.
Fix the approach direction and make it part of the procedure. Zone widths are
per-joint: Joint Y's releases in 2--4\textdegree{} where X's takes 10--17.

\textbf{Trigger position drifts with approach speed.} The same endstop on Joint Y,
approached clockwise twice, reported \texttt{raw} 411950 at speed 50 and 411633 at
speed 25 --- the faster approach recording the trigger further along in the
direction of travel. That is detection-and-stop latency. The bias is small and
systematic, so a joint homed at varying speeds has a reference that moves by
roughly that much. Standardise on one slow speed for the final approach.

Repeatability with both controlled: \textbf{0.046\textdegree{}} across a
165\textdegree{} round trip.

\textbf{Two-phase approach.} Creeping the whole way is slow --- Joint Y's full
envelope takes about 165\,s at speed 25. Running all but the last
15\textdegree{} at speed 150 and creeping the remainder covers it in about
40\,s, with the same precision where it matters. Park the joint clear of the
detection zone afterwards, or it sits on an asserted limit at its own home.

\begin{caution}{Absolute positions do not survive a power cycle}
The encoder counter zeroes at power-on, and the arm may be moved between sessions.
A previously recorded trigger position is a rough expectation, never a target ---
Joint X's endstop was 112\textdegree{} from the start position one session and
173.8\textdegree{} the next. Re-establish the reference by searching, then
\texttt{SET\_AXIS\_ZERO (0x92)} so subsequent readings are joint angles.
\end{caution}

\subsection{When a sensor's LEDs say something impossible}
On a dual-output sensor --- one output per magnet pole, an LED on each --- the
two LEDs are mutually exclusive by physics. A single die cannot be presented
with a north and a south pole at the same point, so \textbf{both LEDs lit at once
is not a detection state}. Neither is both lit and unresponsive to a magnet: that
is both outputs stuck, which is a supply or wiring fault rather than anything
magnetic.

On these modules the indicator usually sits between \texttt{+V} and the output
pin, so it lights when the output is pulled low. Both outputs held low ---
because the chip is not running, or its outputs are damaged, or the pins are
shorted to ground --- lights both LEDs permanently. Things that produce it:

\begin{itemize}
    \item \textbf{Supply reversed or on the wrong pin.} A four-pin dual-output
    module (\texttt{+V}, \texttt{GND}, and two outputs) reversed end-for-end puts
    \texttt{+V} on an output and an output on \texttt{+V}. The chip is then
    fed through a pin that was never meant to carry it, and does not run.
    \item \textbf{Overvoltage.} These are 5\,V parts. On a 42D,
    \texttt{EVCC}/\texttt{EGND} is the board's own 5\,V rated 20\,mA --- two lit
    LEDs plus the die can approach that budget, and anything fed from a 12--24\,V
    rail instead will be damaged outright.
    \item \textbf{Output pins shorted to ground} by a mis-landed wire, which pulls
    both indicators on regardless of what the sensor is doing.
\end{itemize}

\begin{caution}{A reversal damages the part; correcting the wiring does not undo it}
These hall modules have \textbf{no reverse-polarity protection}. Feeding the
supply backwards forward-biases the die's substrate diode, and the usual result
is a destroyed output stage that latches --- which lights both indicators
permanently and never responds to a magnet again. So ``it was plugged in
backwards at first'' is a sufficient explanation on its own, and the symptom
will persist after the wiring is corrected.

What limits the damage is how hard the supply could push. On a 42D,
\texttt{EVCC} is the board's own 5\,V rated 20\,mA, and that current limit may
be enough to have saved a briefly reversed module --- it is worth testing before
assuming the part is dead.

Separate the sensor from the harness by swapping: put a \emph{known-good} sensor
from a working joint onto this joint's harness. If the good sensor works here,
the original is dead and should be replaced. If the good sensor misbehaves here
too, the fault is in the harness or the driver's \texttt{EVCC} rail --- check
that rail directly, since reverse current can damage it as well, and a driver
that is perfectly healthy over CAN can still have a dead 5\,V output.
\end{caution}

Check the supply before suspecting the sensor: measure \texttt{+V} to
\texttt{GND} at the module and confirm it reads about 5\,V with the right
polarity. A sensor whose LEDs respond correctly to each pole \emph{separately} is
working; that is the state to get back to before looking at anything on the CAN
side.
...
Driver stops responding mid-test &
  ...
  once. \\
\textbf{All drivers go silent at once, zero bus-errors} &
  They have lost power; the bus is fine. A real CAN node at the wrong bitrate
  still generates errors, so total silence with a clean counter and the adapter
  still enumerated means nothing is transmitting. If it recurs between runs
  rather than during them, and on a joint drawing almost nothing, suspect the
  supply or its connection rather than current draw --- watch the supply's own
  indicator at the moment it goes to tell a failing supply from failing wiring
  downstream of it. \\
\textbf{Encoder tracks perfectly but the arm does not move} &
  ...
  result. \\
Endstop still silent after enabling \texttt{EndLimit} &
  \texttt{EndLimit} does not make an input \emph{visible}. It governs only
  whether the driver \emph{acts} on a limit. Reading the pin via \texttt{0x34}
  needs no \texttt{EndLimit} at all --- on a 42D the missing piece is almost
  always the limit port remap. \\
...
\texttt{joint\_move.py} &
  \texttt{joint\_move.py z cw 15}. One joint, a named direction, a distance.
  Direction is a name rather than a byte because the byte does not transfer
  between joints; unconfirmed joints are refused rather than guessed at. Endstop
  and stall aborts on by default, \texttt{0xF7} before the coil disable. \\
\texttt{joint\_calibrate.py} &
  \texttt{joint\_calibrate.py z}. Sweeps to both limits at one speed and reports
  the separation. Imports the joint table from \texttt{joint\_move.py} so the
  direction mappings live in one place. Run it twice --- a single figure is not
  a result. \\
\texttt{joint\_roundtrip.py} &
  \texttt{joint\_roundtrip.py 45}, or \texttt{--joints x,z}. Moves each joint out
  and back and reports how far from its start it finished. Skips joints that are
  not on the bus or have no confirmed direction mapping, so it works on a partly
  assembled arm. Flags a leg that stopped early, whose return error describes an
  interrupted move rather than a drivetrain. \\
\bottomrule
\end{longtable}
