...
C & Wrist joint             & \texttt{0x05} & NEMA 17 / SERVO42D & 67.82:1 \\
B & Wrist rotation          & \texttt{0x06} & NEMA 17 / SERVO42D & 67.82:1 \\
\bottomrule
\end{tabular}
\end{center}

\begin{caution}{Joints B and C are documented backwards in several older files}
Joint B is \texttt{0x06}. Joint C is \texttt{0x05}. Files that state the opposite
include \texttt{joint\_b\_final\_reliability\_test.py}, the legacy
\texttt{joint\_b\_micro\_move\_text\_v2.py}, and the old cheat sheet, all of which
hard-code Joint B as \texttt{0x05}. A scan on 2026-09-02 with only Joint B connected
returned a single responder at \texttt{0x06}. Joint A's \texttt{0x04} and its 48:1
ratio are empirically verified; the rest are config values.
\end{caution}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Joint B and C CAN IDs.} Older files hard-code Joint B as
          \texttt{0x05}. Joint B is \texttt{0x06} and Joint C is \texttt{0x05},
          confirmed by scan.
