...
C & \texttt{0x05} & 67.82:1 & Wrist joint \\
B & \texttt{0x06} & 67.82:1 & Wrist rotation \\
\bottomrule
\end{tabular}
\end{center}

\begin{caution}{B and C are documented backwards in older files}
Joint B is \texttt{0x06} and Joint C is \texttt{0x05}. Several files in this project
state the opposite, including \texttt{joint\_b\_final\_reliability\_test.py} and the
old cheat sheet, both of which hard-code Joint B as \texttt{0x05}. A bus scan on
2026-09-02 with only Joint B connected returned a single responder at \texttt{0x06}.
Confirm by scan in Step 3 and believe the scan, not any table --- including this one.
\end{caution}
