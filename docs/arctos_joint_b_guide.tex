...
Joint A & Base rotation & \texttt{0x01} & 500~kbit/s \\
Joint C & Elbow pitch   & \texttt{0x03} & 500~kbit/s \\
...
The \texttt{0x30} form has been used consistently and its response has been decoded and cross-checked over many sessions:

\begin{lstlisting}[language=bash]
cansend can0 005#3035
# Response, e.g.:
# 005#30 00 00 00 00 00 03 39
\end{lstlisting}

Response layout: byte 0 is the command echo, bytes 1--4 are a signed 32-bit \emph{carry} (number of full revolutions since power-on), bytes 5--6 are the \emph{value} (position within the current revolution, \texttt{0}--\texttt{16383}), byte 7 is the checksum.

\begin{lstlisting}[language=python, caption=Correct absolute-position extraction]
carry = int.from_bytes(data[1:5], byteorder="big", signed=True)
value = int.from_bytes(data[5:7], byteorder="big", signed=False)
absolute_position = carry * 16384 + value
\end{lstlisting}

\begin{caution}{Do not use only \texttt{value} as ``the position''}
An earlier version of the test scripts read only the 2-byte \texttt{value} field and treated it as an absolute position. That field wraps every single motor revolution (every 16{,}384 counts), so once the motor turns more than one revolution -- which, given the 67.82:1 gear ratio, happens for almost any real joint motion -- that number alone silently loses track of how many revolutions have occurred. This is the leading suspected cause of an earlier movement test landing far from its intended target. Always combine \texttt{carry} and \texttt{value} as shown above.
\end{caution}

A second opcode, \texttt{0x31}, has also been observed to produce a response (e.g.\ \texttt{31 00 00 00 00 00 03 39}), with a different apparent byte pattern. \textbf{Do not assume this is the same layout as \texttt{0x30}} -- treat it as unconfirmed until its exact meaning is located in \texttt{motor\_types.hpp} / \texttt{motor\_driver.cpp}, and until then, standardize on \texttt{0x30} for all position math in this guide.
...
An early calibration pass, done before the gear ratio was discovered in \texttt{arctos\_controller.yaml}, assumed the motor's raw shaft rotation \emph{was} the joint rotation and produced a figure of ``$\approx$3300 counts per 90$^\circ$.'' That number is real data, but it describes the \emph{motor shaft}, not the joint -- once the 67.82:1 reduction is applied, that same 3300-count motion corresponds to only $\approx 1.07^\circ$ of actual Joint~B travel. Separately, a since-superseded note referenced \texttt{COUNTS\_PER\_REV~=~16050}; the encoder is a 14-bit sensor and the driver's own \texttt{ENCODER\_STEPS~=~16384} constant (i.e.\ $2^{14}$) is both source-verified and the far more plausible round figure. Use only the $\approx 3086.56$~counts/degree formula above for real joint-angle math.
\end{caution}

\begin{caution}{A conflicting ``$-813$ pulses/degree'' figure was also found -- do not use it}
A separate driver draft (\texttt{mks\_driver.py}) defined \texttt{pulses\_per\_degree~=~-813.0} for Joint~B and used it directly as the value written into the CAN frame's position field:
\begin{lstlisting}[language=python]
...
~/arctos_ws/src/ros2_arctos/arctos_motor_driver/
  ...
  src/can_protocol.cpp                          # sendFrame() / checksum -- verify here
...
\begin{enumerate}
    \item Resolve the \texttt{0xF5} absolute-vs-relative question (Section~\ref{sec:motion}) using the micro-move script's empirical output.
    \item Confirm the panel's working mode matches what \texttt{motor\_driver.cpp} assumes (Section~3).
    \item Read \texttt{can\_protocol.cpp}'s \texttt{sendFrame()} to confirm the C++ checksum matches the Python formula verified in this guide.
    \item Only once single-joint CAN control is fully deterministic, wire it into the ROS~2 controller's command topic / action interface.
\end{enumerate}
